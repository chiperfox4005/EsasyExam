...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('hasil_ujian', function (Blueprint $table) {
            // Statistik jawaban
            if (!Schema::hasColumn('hasil_ujian', 'benar')) {
                $table->integer('benar')->default(0)->after('nilai');
            }
            if (!Schema::hasColumn('hasil_ujian', 'salah')) {
                $table->integer('salah')->default(0)->after('benar');
            }
            if (!Schema::hasColumn('hasil_ujian', 'kosong')) {
                $table->integer('kosong')->default(0)->after('salah');
            }
            ...
            // Waktu pengerjaan
            if (!Schema::hasColumn('hasil_ujian', 'mulai_mengerjakan')) {
                $table->timestamp('mulai_mengerjakan')->nullable();
            }
            if (!Schema::hasColumn('hasil_ujian', 'submitted_at')) {
    ...
    public function down(): void
    {
        Schema::table('hasil_ujian', function (Blueprint $table) {
            $table->dropColumn([
                'benar', 'salah', 'kosong',
                'tab_switch_count', 'copy_count', 'paste_count',
                'right_click_count', 'blur_count',
                'mulai_mengerjakan', 'submitted_at'
            ]);
        });
    }
};