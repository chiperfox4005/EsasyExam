...
<div class="max-w-4xl mx-auto space-y-6 animate-fadeIn">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Update informasi kelas</p>
        </div>
    ...
    <form action="{{ route('kelas.update', $kelas->id) }}" method="POST" class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700 mb-1">Nama Kelas</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $kelas->nama) }}" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:outline-none">
                @error('nama') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>
            <div>
                <label for="tingkat" class="block text-sm font-medium text-gray-700 mb-1">Tingkat</label>
                <input type="number" name="tingkat" id="tingkat" value="{{ old('tingkat', $kelas->tingkat) }}" min="1" max="12" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div>
                ...
                <select name="tipe" id="tipe" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <option value="formal" {{ old('tipe', $kelas->tipe) == 'formal' ? 'selected' : '' }}>Formal (Sekolah)</option>
                    <option value="virtual" {{ old('tipe', $kelas->tipe) == 'virtual' ? 'selected' : '' }}>Virtual (Les Online)</option>
                </select>
            </div>
            <div>
